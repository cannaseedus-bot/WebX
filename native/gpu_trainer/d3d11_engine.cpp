...
bool D3D11Engine::init(bool forceWarp, bool verboseLog) {
    if (std::getenv("KUHUL_D3D11_DEBUG") == std::string("1"))
        std::cerr << "D3D11 init: CreateDevice begin\n";
    UINT flags = 0;
#ifdef _DEBUG
    ...
    if (!forceWarp) {
        ...
        if (SUCCEEDED(hr)) {
            if (std::getenv("KUHUL_D3D11_DEBUG") == std::string("1"))
                std::cerr << "D3D11 init: hardware device ready\n";
        ...
        if (SUCCEEDED(hr)) {
            if (std::getenv("KUHUL_D3D11_DEBUG") == std::string("1"))
                std::cerr << "D3D11 init: WARP device ready\n";
            if (verboseLog)

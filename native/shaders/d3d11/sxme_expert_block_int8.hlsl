cbuffer ExpertBlockInt8Params : register(b0)
{
    uint batchSize;
    uint hiddenSize;
    uint blockRows;
...
{
    uint byteIndex = elementIndex;
    uint word = weightBlock.Load(byteIndex & ~3u);
    uint encoded = (word >> ((byteIndex & 3u) * 8u)) & 0xffu;
    int signedValue = (encoded >= 128u)
        ? int(encoded) - 256
        : int(encoded);
...
[numthreads(64, 1, 1)]
void CSExpertBlockInt8Gemm(uint3 id : SV_DispatchThreadID)
{
    uint flat = id.x;

...
void HashConcat8x8(uint a[8], uint b[8], out uint digest[8])
{
    ...
    SHA256_IV(digest);
    SHA256_Block(block, digest);
    for (uint i = 0; i < 16; i++) block[i] = 0u;
    block[0] = 0x80000000u;
    block[15] = 512u;
    SHA256_Block(block, digest);
}
...
void HashConcat8x8x8(uint a[8], uint b[8], uint c[8], out uint digest[8])
{
    ...
    SHA256_IV(digest);
    SHA256_Block(block, digest);
    for (uint i = 0; i < 8; i++) block[i] = c[i];
    for (uint i = 8; i < 16; i++) block[i] = 0u;
    block[8] = 0x80000000u;
    block[15] = 768u;
    SHA256_Block(block, digest);
}
...
{
    // VM-2 records gate validity but never blocks COMPUTE_FOLD execution.
    bool gate_valid = ControlFlags[0] == cm1_gate;
    ...
    else if (is_merkle_node)
    {
        ...
        if (tid == 0 && gs_merkle_ok == 1u && gate_valid)
        {

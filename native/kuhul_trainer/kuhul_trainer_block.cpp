// ===========================================
// K'UHUL NATIVE TENSOR TRAINER
// 3D & 8D Tensor Operations - No PyTorch
// ===========================================
...
public:
    ...
    Tensor3D& operator=(Tensor3D&& other) = default;
...
template<typename T> class Tensor4D; // forward declare 4D used by entangle
...
public:
    ...
    Tensor3D<T> contractTo3D(size_t keep1, size_t keep2, size_t keep3) const {
...
public:
    ...
    void update(TensorType& params, const TensorType& gradients) override {
        ...
        auto& paramData = params.getData();
        // simple Adam-like update (placeholder)
        for (size_t i = 0; i < paramData.size(); ++i) {
            m[i] = beta1 * m[i] + (1 - beta1) * gradData[i];
            v[i] = beta2 * v[i] + (1 - beta2) * (gradData[i] * gradData[i]);
            double m_hat = m[i] / (1 - std::pow(beta1, static_cast<double>(t)));
            double v_hat = v[i] / (1 - std::pow(beta2, static_cast<double>(t)));
            paramData[i] -= learningRate * m_hat / (std::sqrt(v_hat) + epsilon);
        }
...
int kuhul_main() {
    ...
    for (int i = 0; i < 1000; ++i) {
        Kuhul::Tensor3D<float> input(8, 8, 8);
        Kuhul::Tensor3D<float> target(8, 8, 8);
    ...
    // Train model (requested run)
    std::cout << "\nStarting training (1000 samples, 2 epochs)...\n";
    auto start = std::chrono::high_resolution_clock::now();
    
    model.train(dataLoader, 2);
...
int main() { return Kuhul::kuhul_main(); }

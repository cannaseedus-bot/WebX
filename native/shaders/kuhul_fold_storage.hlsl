...
RWStructuredBuffer<uint>  ReplayRingMeta  : register(u4);  // slot_id, state
RWStructuredBuffer<float> RetrievedBuffer : register(u5);  // non-destructive retrieval output
RWStructuredBuffer<uint>  SnapshotState   : register(u6);  // 0 empty, 1 dirty, 2 sealed
...
{
    // Storage uses its own fold dispatch gate; invalid work is not committed.
    ...
    // -----------------------------------------------------------------------
    else if (op == OP_SEAL && slot < num_snap_slots &&
             SnapshotState[slot] == 1u)
    {
        if (tid == 0)
        {
    ...
    else if (op == OP_GET && slot < num_snap_slots)
    {
        ...
        // Retrieval is non-destructive: never modify the sealed snapshot.
        if (verified == 1u)
        {
            uint dst_base = slot * slab_size;
            for (uint i = tid; i < slab_size; i += 64)
                RetrievedBuffer[dst_base + i] = SnapshotBuffer[dst_base + i];
        }
    }
    ...
    else if (op == OP_REPLAY && node_id >= 100)
    {
        ...
        if (tid == 0)
        {
            ...
            InterlockedCompareExchange(
                ReplayRingMeta[meta_idx + 1], 0u, 1u, prior_state);
            gs_verified = prior_state;
        }
        GroupMemoryBarrier();

        if (gs_verified == 0u)  // this group claimed the slot
        {
            ...
            if (tid == 0)
            {
                ...
                // The dispatch boundary closes the write before readers run.
                // State 1 means claimed and committed; later writes are refused.
            }
        }
    }
}

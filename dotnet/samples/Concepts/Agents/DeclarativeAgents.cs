...
public class DeclarativeAgents(ITestOutputHelper output) : BaseAgentsTest(output)
{
    ...
    [InlineData(true)]
    [InlineData(false)]
    public async Task LoadsSemanticAgenticModelManifest(bool useChatClient)
    {
        var agentFileName = "SemanticAgenticModel.yaml";
        var manifestLookupDirectory = Path.Combine(Directory.GetCurrentDirectory(), "..", "..", "..", "Resources", "DeclarativeAgents");
        var manifestFilePath = Path.Combine(manifestLookupDirectory, agentFileName);
        var manifestText = await File.ReadAllTextAsync(manifestFilePath);

        Assert.Contains("type: function", manifestText);
        Assert.Contains("type: openapi", manifestText);
        Assert.Contains("type: code_interpreter", manifestText);
        Assert.Contains("type: file_search", manifestText);
        Assert.Contains("DuckDuckGo-backed web search", manifestText);
        Assert.Contains("WebSearchPlugin.Search", manifestText);
        Assert.Contains("agentic_models:", manifestText);
        Assert.Contains("function-model", manifestText);
        Assert.Contains("openapi-model", manifestText);
        Assert.Contains("code-interpreter-model", manifestText);
        Assert.Contains("file-search-model", manifestText);
        Assert.Contains("web-search-model", manifestText);
        Assert.Contains("duckduckgo-web-search", manifestText);
        ...
        Assert.NotNull(agent);
        Assert.Equal("SemanticAgenticModel", agent.Name);
        Assert.NotNull(agent.Description);
        Assert.NotEmpty(agent.Description);
        Assert.NotNull(agent.Instructions);
        Assert.NotEmpty(agent.Instructions);
        Assert.NotNull(agent.Metadata);
        Assert.NotNull(agent.Metadata.ExtensionData);
        Assert.True(agent.Metadata.ExtensionData.ContainsKey("agentic_models"));

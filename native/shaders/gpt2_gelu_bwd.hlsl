...
// t0 = pre_gelu; u0 = d_mlp_buf (in-place: reads d_mlp_gelu, writes d_mlp_pre)
// In-place avoids the D3D11 SRV/UAV aliasing that silently nulled the old t1 dout binding.
StructuredBuffer<float>   pre_gelu : register(t0);  // x before GELU [numel]
RWStructuredBuffer<float> dx       : register(u0);  // in-place: d_mlp_gelu → d_mlp_pre
...
void CSMain(uint3 tid : SV_DispatchThreadID) {
    ...
    dx[i] *= dgelu_dx;  // in-place: d_mlp_pre = d_mlp_gelu * gelu'(pre)
}
